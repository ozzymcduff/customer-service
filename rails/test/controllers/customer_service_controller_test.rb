...
<Customer>
  ...
  <AddressCity>Malmoe</AddressCity>
  <AddressCountry>Sweden</AddressCountry>
  <AddressStreet>Lugna gatan 40</AddressStreet>
...
class CustomerServiceControllerTest < ActionDispatch::IntegrationTest
  test "should get all customer" do
    get "/CustomerService.svc/GetAllCustomers", params: { "CONTENT_TYPE" => "application/xml", format: "xml" }
    assert_response :success
    assert_equal($get_all_customers.strip!, response.body.strip!)
  end
  ...
  test "should be able to save customer" do
    ...
    assert_equal("Malmö", oskar.address_city)
    assert_equal("Sweden", oskar.address_country)
    assert_equal("Lugna gatan 40", oskar.address_street)
  end
end
